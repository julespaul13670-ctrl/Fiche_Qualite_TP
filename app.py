...
# 2. On enchaîne obligatoirement avec ELIF
    ...
    if not liste_chantiers:
        ...
        if chantier != "Sélectionner...":
            ...
            # --- 2. SÉLECTION DU CONTRÔLEUR ET DATE ---
            c1, c2 = st.columns(2)
            with c1:
                ...
                if choix_nom == "Autre...":
                    ...
                    nom_final = choix_nom if choix_nom != "Sélectionner..." else ""
            with c2:
                date_auto = st.date_input("📅 Date", datetime.now())

            st.divider()

            # --- 3. CHARGEMENT CONFIGURATION EXCEL ---
            df = st.session_state.df_config
            if df is None or df.empty:
                st.error("Le fichier de configuration des questions est introuvable.")
            else:
                # Préparation de la liste des ouvrages
                ...
                if ouvrage_sel != "Sélectionner...":
                    # --- 4. GESTION DU CAS 'AUTRE' VS NORMAL ---
                    if ouvrage_sel == "Autre":
                        ...
                        st.info("💡 Remplissez les contrôles généraux ci-dessous.")
                    else:
                        df_ouv = df[df['Ouvrage'] == ouvrage_sel]
                        ouvrage_final_nom = ouvrage_sel

                    # --- 5. SOUS-CATÉGORIES / TYPES ---
                    ...
                    if not df_scat.empty:
                        ...
                        if sc_sel != "Sélectionner...":
                            ...
                            for i, row in df_filtre_sc.iterrows():
                                ...
                                if pd.isna(question_texte) or str(question_texte).strip() == "": continue
                    ...
                    # --- 6. QUESTIONS SPÉCIFIQUES ET GÉNÉRALES ---
                    if (df_scat.empty) or (not df_scat.empty and sc_sel != "Sélectionner..."):
                        # Points de contrôle de l'ouvrage
                        ...
                        # --- 7. OBSERVATIONS ET PHOTO ---
                        commentaire = st.text_area("📝 Observations particulières", key="comm_zone")
                        photo = st.file_uploader("📸 Justificatif Photo", type=['png', 'jpg', 'jpeg'], key=f"upload_{ouvrage_sel}")
                        
                        if photo:
                            st.image(photo, caption="Aperçu", width=300)
                            st.session_state['temp_photo_bytes'] = photo.getvalue()

                        # --- 8. GÉNÉRATION PDF ---
                        if st.button("🚀 1. Générer l'Aperçu"):
                            if not nom_final:
                                st.error("Veuillez indiquer le contrôleur.")
                            else:
                                # (Ta logique de génération PDF ici...)
